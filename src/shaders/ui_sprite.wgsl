...
fn vs_main(
    ...
    let uv_x = instance.uv_rect.x + p.x * instance.uv_rect.z;
    let uv_y = instance.uv_rect.y + p.y * instance.uv_rect.w;
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    if (in.use_texture > 0.5) {
        let tex_color = textureSample(texture_atlas, texture_sampler, in.uv, i32(in.atlas_index));
